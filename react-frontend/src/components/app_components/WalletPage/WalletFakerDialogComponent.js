...
import walletFakerFactory from "./walletFakerFactory";
...
const WalletFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = walletFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("wallet").create(elem));
...
export default connect(mapState, mapDispatch)(WalletFakerDialogComponent);
