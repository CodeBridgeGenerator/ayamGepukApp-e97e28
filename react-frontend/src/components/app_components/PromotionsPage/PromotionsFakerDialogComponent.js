...
import promotionsFakerFactory from "./promotionsFakerFactory";
...
const PromotionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = promotionsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("promotions").create(elem));
...
export default connect(mapState, mapDispatch)(PromotionsFakerDialogComponent);
