...
const UserDetailsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,userId: _entity?.userId,phoneNumber: _entity?.phoneNumber,email: _entity?.email,passwordHash: _entity?.passwordHash,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                <label htmlFor="passwordHash">password_hash:</label>
